...
36. [uxai-app-entry-routing.md](uxai-app-entry-routing.md) — UXAI 前端**两条平行入口链**:浏览器/Playwright 走 `entry.tsx → app.tsx`,Electron 走 `desktop/renderer → @opencode-ai/app → octoapp/index.ts → octo.tsx`(HashRouter,无地址栏)/ **更正旧结论**:`octo.tsx` 不是死副本,反而是维护更勤的一份(45 vs 14 提交),`app.tsx` 是掉队分叉(缺 ForceLightScheme·Pattern 路由·onboarding·多种侧栏布局)→ 同页两端壳不同,拿浏览器验 UI 会被误导 / 误判成因:引用发生在跨 package re-export 链上,单目录 grep 看不见 / 方法论修正:顶层探针只能证明"不在 X 入口的图里",不能证明"死文件" / dev sandbox 路由要两端都挂 + 两端都走裸壳
