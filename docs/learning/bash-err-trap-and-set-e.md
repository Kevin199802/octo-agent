...
`probe_asset` 是在 `probe_asset … || bad=…` 里调用的 —— 按第 3b 条，**父层的 ERR trap 在整个函数体内被豁免**，所以父层不吭声；而命令替换的子 shell 不在那个豁免范围里，`-E` 让它照样继承 trap。于是**只有子 shell 那一次触发**，输出直接进变量，终端上看不到任何异常。

> 换个上下文行为就不同：同样的赋值放在**顶层**，触发的是父层那次 trap，脚本当场带着契约行退出 —— 响亮、好查。同一行代码，放在哪决定了它是"响亮失败"还是"静默污染"。
...
| 让某个 `VAR="$(cmd)"` 里的 `cmd` 真失败一次 | 变量里**只有 cmd 的输出**，没有契约行；`$?` 是 cmd 的真实码 ← 第 5 条就是这条挂 |
