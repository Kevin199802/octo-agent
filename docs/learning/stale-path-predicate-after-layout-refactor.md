...
| 可观测 | **完全静默**:无异常、无 catch、无日志 | `rename` ENOENT → 走 catch → 打 `upload-move failed` |
| 结果 | 半年无人报 | 当天就会被日志发现 |

代码里其实有完备的日志:`upload-copy ok` / `upload-move ok` / `upload-move failed`。
但**恒假走的是"连尝试都没有"的那条路,三条日志一条都不打**。
现场证据就是:复现时控制台只有 `upload-copy ok`,**没有 `upload-move ok`,也没有 `upload-move failed`**
—— "该出现的日志没出现"比"出现了错误日志"难注意得多。
...
3. **恒假的守卫是死分支,而死分支不报错。** 判据类代码值得问一句:"如果它永远返回 false,
   我从哪看出来?"答不上来就补日志或补测试。
4. **纯函数别住在组件里。** 判断逻辑一旦超出"一眼看穿",就搬进 `utils/` 导出 + 单测 ——
   住错地方等于自愿放弃回归保护。
5. **同一能力的两条实现路径,坏一条时不会有人报"坏了"。** 排查时先问"这个功能有几条路径进来",
   而不是默认只有一条。
