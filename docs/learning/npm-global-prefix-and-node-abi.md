...
- **取消一个依赖，要取消干净才有收益。** 这次除了不下载 node，还必须**不去读远端 manifest**
  —— 否则"绕开曾经 504 的那条链路"只绕开了一半，仍然会在同一个网关上卡住。顺带 `python3`
  也从 macOS 的硬门槛降为"只有要解析 manifest 时才需要"（本地 JSON 有 node 就用 node 解析）。
- **但"换一个本地来源"要换对文件 —— 这次换错了，而且错得很贵**（2026-09-14 补记）。
  起草时把装 yarn 的 registry 回落到了 **`template/.npmrc`**，理由是"它也是本地的、还与依赖树同版本"。
  可那是**项目依赖源**，上面根本没有 `yarn` 这个包；装 yarn 要的是**通用 npm 镜像**。
  于是复用系统 node 的机器——也就是这条改动想服务的大多数机器——首装必挂在 `npm i -g yarn`。
  正确的本地来源是 skill 自带的 `references/env.manifest.json` 的 `npmRegistry`：同样随包走、
  同样不走网络，而它本来就是"装 yarn 的源"。**教训不是"别用本地来源"，是"两个源长得像不等于能互换"**
  ——判断一个源能不能替代另一个，要看**它上面有没有你要装的那个包**，不是看它在不在本地。
  详见 [SPEC-DES-001 §4.1「③④ 的 registry 必须分开处理」](../specs/design/fastui-vue-codegen-pipeline.md#41-v1-路线手上有能跑的-node-就用它一个都没有才分发-portable-node)。
