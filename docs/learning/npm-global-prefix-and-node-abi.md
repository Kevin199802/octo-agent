...
### 2.1 那要不要设个版本门禁？——我们设了一版白名单，最后**去掉了**

上面第 2、3 类风险只在**没验过的新大版本**上出现。顺着这个思路，第一版写的是一条大版本白名单
（`systemNodeMajors = [18, 20, 22]`，命中才复用，命不中退回下载我们定版的那个 node），理由听起来很正当：
"没验过的版本不该算满足要求"。

**定案时整条去掉了。** 三个理由，值得记住的是第一条：

1. **那个名单本身是猜的。** `22` 来自我们托管的 portable 包、`20` 来自"内网普遍是 20/22"、`18` 是还算常见的旧 LTS；
   排除 24/26 **不是因为验过它们坏，只是因为没人验过**。—— 一条判据的**两侧都要有依据**：
   "命中"那侧有（这几个版本跑过），"命不中"那侧没有（不知道它坏，只是不知道它好）。
   **拿没有证据的猜测当门禁，是用确定的代价换不确定的风险。**
2. **代价落在一台什么都不缺的机器上。** node 24 的机器装 yarn、装依赖全都没问题，却会因为一个猜出来的数字
   被推去走一条**已知被 WAF 403 过**的下载链路，然后死在那儿。
3. **真不兼容可以在事后精确认出来，不必事前猜。** 白名单想拦的那两类断裂（webpack md4 / 原生模块 ABI）
   都以**编译期报错的固定文本**出现（`ERR_OSSL_EVP_UNSUPPORTED`、`digital envelope routines`、
   `NODE_MODULE_VERSION`…）。于是判据从"事前猜版本号"挪成"**事后认指纹**"：命中就打一行
   `NODE_SUSPECT`，明确写上"这不是你写的代码的问题"。

**一般化**：想设门禁时先问一句 ——「**命不中的那一侧，我有证据吗？**」没有的话，通常存在一个把判断
**推迟到有证据时**的办法（这里就是编译期指纹）。推迟不了、或漏网的后果不可承受，才轮到用猜测挡住所有人。

> 代价要诚实说：漏网的那部分会以"编译失败"的形态出现，多耗一轮。所以**补偿控制必须真的到位** ——
> 指纹要落盘、要写进给模型看的 SKILL.md，否则模型只会看到一条普通编译错误，拿着它去改自己的代码死循环。
> 「不设门禁」和「有一条能认出它的指纹」是一对，只做前半是放任。
...
| 运行时能跑起来 | `node -v` 跑得出来（不设版本门禁）+ 编译期指纹事后指认 | 不靠 lockfile |
