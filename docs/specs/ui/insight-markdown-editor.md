...
| 风险 | 兜底 |
|---|---|
| 自动保存把误删/误改立即写盘 | ① 编辑器内撤销栈（Vditor 自带，Cmd/Ctrl+Z）；② **进编辑器时存一份原始内容快照**，顶栏提供「还原初始内容」 |
...
保留 Vditor **较完整**工具栏：标题、加粗/斜体、列表、引用、表格、链接、代码块、**公式 / 流程图 / 图表 / 脑图**、撤销/重做、模式切换（sv/ir/wysiwyg）、大纲、预览、导出。仅去掉与我们体系冲突/无关的：上传（走自有附件体系）、录音、`@`提及。

### 6.4 主题

跟随 octo 明暗（`--octo-*` token / 现有 dark 判断），`setTheme("dark"|"classic")` + 预览主题对应切换。
...
- 写盘路径双重校验（渲染进程 `isValidUserPath` + 主进程白名单），文件名 `sanitizeFilename` 去分隔符
...
- **B 分屏 + 同步滚动**：默认 `sv` 左右分栏；左改右即时更新；滚动同步；切 `ir`/`wysiwyg` 正常
- **C 自动保存**：编辑停手约 1s → 顶栏 `保存中…`→`已保存`；到 `<projectDir>/.octo/downloads/` 下确认**本地 .md 文件真被覆盖写入**且内容一致；重开同卡 / 用「本地应用打开」看到的都是这一份；重开编辑器内容延续
- **D 兜底**：① 误删后 Cmd+Z 可撤销 + 「还原初始内容」可回到进入时快照；② mock writeFile 拒绝 / 文件被占用 → toast「保存失败」+ 状态红 + 内容不丢；③ projectDir 为空 → 提示并禁用
...
console 埋点：`[octo:mdedit] open` / `save-start` / `save-ok` / `save-failed`（含 path + bytes）/ `restore` / `close`。
