...
- **技能**：`window.api.getSkillContent(name)` 拿 SKILL.md →
  ...
  ```
- **文件**（不用 `[附件]` 前缀，避免 InsightTurn 误渲染成文件卡片——这是另起一个头的**唯一**理由，见 §8）：
  ```
  [引用文件] 用户本轮 @ 引用了以下已存在的会话文件(与 [附件] 同属本会话可用文件):
  - {filename}: {绝对path}
  ```
  客户端单一事实源 `lib/upload.ts` `formatMentionedFilesForPrompt`（正常发送与排队 drain 共用）。
  **文案只陈述事实、不含工具指令**（2026-08-20 修正）：初版写「需要时用 `extract_document` 按路径读取」，与 chip turn 禁用 `extract_document` 的模板指令直接冲突；怎么读由常驻提示词统一交代。

  **`@` 的文件与附件走同一条内联路径**（2026-08-20 修订，见 §8）：清单只负责「白名单 + 不暴露路径」，正文由 SPEC-INS-015 路由 ① 的 `FilePart(file://)` 内联，与附件栏文件完全一致。两个入口合并后传入 `assembleInsightParts` 的 `textInlineFiles`，按 path 去重。
...
- **chip（MCP，SPEC-INS-017）**：技能注入是 synthetic 前缀、chip 是 turn 级工具 gate，正交、可共存，不做互斥。
  - ~~"正交、互不影响"对 `@文件` 不成立~~（**2026-08-20 内网修复**）：初版只验了 `@技能` × chip，漏判了 `@文件` × chip。`[附件]` 当时是 MCP 文件白名单的**唯一**载体（octo-upload-inject 只按 `[附件]` 头聚合清单，chip 模板的「文件引用铁律」也只承认它），于是「先对话生成一个 md → `@` 它 → 选研究工具触发解析」**必死**：模型读到铁律发现该文件不在 `[附件]` 里而自我阻断（内网实测）；就算硬填，插件 `resolvePath` 也会 miss 抛错——两条路都堵。
  - 根因是**契约漏配而非安全设计**：`[附件]` 成为白名单唯一载体纯属时间顺序（017 落地时 `@` 引用还不存在），023 引入第二个文件来源时只对齐了「读」（`extract_document`），没对齐「传」（MCP 白名单）。
  - **修复**：插件 `MANIFEST_HEADERS` 同时收 `[附件]` / `[引用文件]`，chip 模板与常驻提示词的措辞改为「会话文件清单」。白名单语义不变（只认清单内、精确匹配、miss 响亮失败）。业界对照：MCP 自身的 Resources（`resources/list` 声明可用资源，模型只能引用清单内 URI）、Files API 的 file_id —— **入口可以多样，注册表必须唯一**。
  - **为什么不合并成单一区块头**：session 消息持久化，旧会话里永远是 `[附件]`，插件与 UI 必须永久保留旧头解析 → "统一成一个头"的收益拿不到，只会变成三态。双头 + 共享头列表常量才是正解。
- **附件**：`@文件`（引用已存在会话文件）与附件上传（新增文件）是两条**采集**路径，但产出的清单对下游（MCP 白名单）等价，见上条。
- **`@` 的文件改为与附件一致地内联正文**（2026-08-20 修订）：
  - **原状**：`@` 只给路径、让模型调 `extract_document` 读；`[附件]` 的 txt/md 则走 FilePart(`file://`) 由服务端 `read` 内联。同一个 md，从附件栏加就是内联、用 `@` 加就得多跑一轮工具——spec 初版未论证过这个差异，实为默认继承了「给路径让模型读」。
  - **业界对照**：`@` 引用**发送即注入上下文**是通行做法（Cursor `@file`、Copilot `#file`、Claude Code `@`），上游 opencode 自己也是（[build-request-parts.ts](../../../packages/app/octoapp/components/prompt-input/build-request-parts.ts) 构造 `FilePart(file://…, text/plain)`，服务端 [prompt.ts](../../../packages/opencode/src/session/prompt.ts) 调 `read` 内联）。「给路径让模型自己读」那套服务的是 **agent 自主探索大型代码库**（模型不知道该看哪个文件时），不是用户显式引用的处理方式——用户 `@` 了就是明确说「这个你看」，再多跑一轮工具纯属浪费。故本次是**回归上游机制**，非新发明。
  - **chip turn 不做特殊处理**（起草时曾计划「chip turn 关闭内联」，核实后撤回）：① FilePart 内联只涉及文本类，office 根本不走这条路；② 内联走 `read`，单文件 2000 行 / 50KB 截断，不是无限回灌；③ §8-9（2026-08-19）那次的上下文炸弹源头是 `extract_document` 对 office **全文回灌**（SPEC-INS-016 §4.1，一叠逐字稿 ≈17 万 token），与本路径不是同一条；④ 内联只发生在 `@` 的**那一轮**（mentions 只在当轮注入）。而代价很大：**chip 是纯常驻的**，关掉内联意味着用户选中研究工具后的所有对话都失去文件正文，叠加 `extract_document` 已禁 → 模型对文件内容彻底失明。
  - **残留风险（已知、接受）**：`@` 十份大 md ≈ 500KB ≈ 12 万 token，128K 模型会爆。概率低且**失败可见**（模型直接报上下文超限）。策略是先按标准做法做，真在内网撞上再针对性处理（如按本轮内联总量超阈值降级），不预防性一刀切。
  - **配套**：常驻提示词的「禁止对已内联文件重复 `extract_document`」硬规则，判据从「在不在 `[附件]`」改为「**在不在清单区块里 + 是不是 office/pdf**」，两个清单一视同仁——这也消化了修复 A 时那个「两清单在传上等价、在读上不等价」的别扭区分。
