...
| V6.1 | **带图对话的图片不重复**（2026-08-18 内网实测发现并修复） | 打开一条**当年发过图**的 chat 历史 | 每张图**只出现一次**（修复前是每张画两遍：2 张图显示成 4 个缩略图）；点击缩略图能放大预览；缩略图尺寸与 insight 自己发的图一致（48×48 圆角）。<br>成因见 [SPEC-INS-030 §6](../agents/insight-knowledge-search.md)：chat 的图是内联 base64（`data:` URL），正好命中上游 `Message` 的 `attached()` 分支，与 insight-turn 自己那层重叠；insight 自己的图走 S3（`https`）命不中，所以长期没暴露。<br>⚠️ 这条**必须用真实 chat 历史验**，insight 自己发图复现不出来 |
