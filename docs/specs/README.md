...
| 015 | Insight Agent 文件传参机制（上传 / 路由）；**2026-09-02 修订**：③ 图片去 S3，改走本地路径 + 服务端读盘转 base64（[ADR-017](../adr/017-insight-image-local-path-base64.md) 推翻 ADR-015 决策 2 在 insight 的适用——opencode server 是本机 sidecar） | 草案；①②④ 已落地，③ 改版实现在 UXAI [#754](https://github.com/MyHeavenDyf/UXAI/pull/754)（评审中，图片大小上限待补） | infra/insight/agent | [insight-file-passing.md](infra/insight-file-passing.md) |
