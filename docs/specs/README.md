...
| 033 | Insight 统一产物统计打点（`artifact-output`）— 现有三个 `artifact-*` 事件基于客户端解析 tool part，`bash` 产的文件永远漏报；改用服务端 git snapshot 的 `UserMessage.summary.diffs` 补一条 turn 级总量事件，与现有三条互补不替代。含对 UXAI 侧初稿的评审修订（触发时机 / baseline / 路径过滤三处会导致数据失真的错误） | 草案（待实现） | infra/insight | [insight-artifact-output-tracking.md](infra/insight-artifact-output-tracking.md) |
