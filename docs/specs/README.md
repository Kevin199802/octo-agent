...
| 033 | Insight 统一产物统计打点（`artifact-output`）— 现有三个 `artifact-*` 事件基于客户端解析 tool part，`bash` 产的文件永远漏报；改用服务端 git snapshot 的 `UserMessage.summary.diffs` 补 turn 级总量事件，与现有三条互补不替代。含对 UXAI 侧初稿的评审修订（触发时机 / baseline / 路径过滤三处会导致数据失真的错误）与两个决策：**D2（2026-08-29）四条 `artifact-` 事件统一改 per-file 粒度**（面板按行数统计不看 extend.count）、**D3（2026-08-29）发射器迁服务端**（opencode `tracking/report.ts`，summarize 落库后直发——「tracker SDK 纯浏览器发不出去」经查证是伪前提，前端 effect 与 baseline/守卫/debounce 三层补丁全删，切走会话漏报根除） | **已实施**（服务端形态，UXAI feature/insight-artifact-tracking） | infra/insight | [insight-artifact-output-tracking.md](infra/insight-artifact-output-tracking.md) |
