...
> 状态：草案（v18，修订记录改为可读结构、决策回填正文；§4.4 / §8.6 已拆出为 SPEC-DES-002 / 003；§6 端口分配已被 SPEC-DES-004 取代） · 优先级 P1 · 规模 [L] · 领域 infra/design
>
...
> ⚠️ **本节的端口分配已被 [SPEC-DES-004](fastui-preview-identity.md) 取代**(2026-09-17):端口不再由 `new-session` 分配、不再写进 `.octo-fastui.json` 与预览卡片,改为点击卡片时由宿主在 spawn 那一刻挑选;卡片只记产物(`fastui://<产物名>`)。起因是「前一个对话的预览卡片点进去变成后一个对话的页面」。本节保留为历史记录,**实现以 004 为准**。
