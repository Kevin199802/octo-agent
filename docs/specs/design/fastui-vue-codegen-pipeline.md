...
| 不下载 node 时**不读 manifest**；装 yarn 的 registry 回落到 **skill 自带 `references/env.manifest.json` 的 `npmRegistry`**（不走网络）。**不是 `template/.npmrc`** —— 那是项目依赖源，装 yarn 会报错（2026-09-14 复核修正，见 [§4.1](#41-v1-路线手上有能跑的-node-就用它一个都没有才分发-portable-node)） | 同上 |
...
| **`install.ps1` 的 v15 + v16 改动**（v15：S5 落盘 / S8 兜底 catch / `-Check`；v16：node 来源决策 / 只在要下载时读 manifest / 交棒用 `$EffectiveNode`） | 本机没有 pwsh，只做了静态检查：UTF-8 with BOM 仍在、括号平衡、`Fail` 等函数全部定义在调用点之前。**逻辑一次都没跑过**，Windows 上第一次跑要盯着看 —— 对照的 macOS 侧同样几条路径已本地实测通过（v16 见 [§9.1](#91-外网可做本地-mac) V0-i；2026-09-11 跑的是八项，2026-09-14 新增的第 9 条单独跑过，整套未在新 fixture 下重跑）。v16 的 Windows 侧另有两处只有 PowerShell 才验得了的点：`Get-Command node -CommandType Application` 取到的 `.Source`、以及 `& $NodeBin -v` 在拿错平台包时是抛异常还是只给非零退出码（两条都按"抛也接住"写了，但没实跑过） |
...
> **"分开处理"是双向的 —— 反过来拿 ④ 的源去做 ③ 同样不行。**（2026-09-14 复核，v16 起草时踩过）
>
> 这两个 registry 服务的是两件事，**互相都不能替代**：
>
> | | 是什么 | 有没有 `yarn` 这个包 |
> |---|---|---|
...
>
> v16 起草时把③的回落源写成了 `template/.npmrc`，理由是"不读 manifest 就得有个本地来源"。
> 本地来源这个方向是对的，**选错了文件**：该用的是 skill 自带的
> [`references/env.manifest.json`](../../../skills/fastui-vue-creator/references/env.manifest.json) 的 `npmRegistry` —— 它同样随 skill 包走、同样不走网络，
> 但它本来就是"装 yarn 的源"。照起草稿实现的话，**复用系统 node 的机器（v16 的主路径）
> 首装必挂在 `npm i -g yarn` 这一步**，而那正是 v16 想服务的大多数机器。
>
> 于是③的回落链定为：`--registry`（下载路径从远端 manifest 取）→ `OCTO_NPM_REGISTRY`
> → **skill 自带 manifest 的 `npmRegistry`** → 本机 npm 配置。`template/.npmrc` **不在这条链上**。
