...
> 状态：草案（v18，修订记录改为可读结构、决策回填正文；§4.4 / §8.6 已拆出为 SPEC-DES-002 / 003；§6 的端口所有权缺陷与修法拆出为 SPEC-DES-004） · 优先级 P1 · 规模 [L] · 领域 infra/design
>
...
| 不下载 node 时**不读 manifest**；装 yarn 的 registry 用 `setup-env.mjs` 里写死的 `YARN_REGISTRY`（通用 npm 镜像）。**不是 `template/.npmrc`** —— 那是项目依赖源，装 yarn 会报错（2026-09-14 复核修正，见 [§4.1](#41-v1-路线手上有能跑的-node-就用它一个都没有才分发-portable-node)） | 同上 |
...
> 本地来源这个方向是对的，**拿错了文件**：那是④的源，上面没有 `yarn` 这个包。照起草稿实现的话，
> **复用系统 node 的机器（v16 的主路径）首装必挂在 `npm i -g yarn` 这一步**，
> 而那正是 v16 想服务的大多数机器。
>
> 改法是**把通用镜像地址写死在 `setup-env.mjs` 里**（`YARN_REGISTRY`）：它是内网的一个固定地址，
> 改它无论如何都要发一版新 skill 包，放进配置文件只是多一层读取和一条"文件坏了"的失败路径。
> 于是③的取值是：`--registry`（安装脚本从远端 manifest 取到时会传，也是人工逃生口）
> → `OCTO_NPM_REGISTRY` → `YARN_REGISTRY`。`template/.npmrc` **不在这条链上**。
...
> ⚠️ **本节的机制在单个工作目录内成立，但不足以支撑多对话并发**（2026-09-14 用户实撞 + 外网实测）——端口所有权在「`.ports` 占位表 / `.octo-fastui.json` 的 `port` / 真实 TCP 监听」三处各记一份且从不同步，表现为「前一个对话的预览卡片点进去变成后一个对话的页面」。缺陷分析、修法与验证清单见 **[SPEC-DES-004](fastui-preview-identity.md)**。本节保留为机制描述，其中 **§6.2.1 的原子占位依然有效**（004 只把它的作用域从「一个工作目录」提到「一台机器」，不推翻）。
