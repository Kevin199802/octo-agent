...
| `[octo:attach]` | [utils/build-prompt-parts.ts](../packages/app/octoapp/pages/insight/utils/build-prompt-parts.ts) 判定 · [index.tsx](../packages/app/octoapp/pages/insight/index.tsx) / [utils/queue-drain.ts](../packages/app/octoapp/pages/insight/utils/queue-drain.ts) 打点 | **SPEC-INS-032 §2.3 内联分层**(发送前按总字节确定性判定,不交给模型):`内联预算超限,转子代理分治`(count/totalBytes/budget/oversized/unknownCount)= 本轮文本材料超 `INLINE_BUDGET`(32KB),**一个 text/plain FilePart 都不产**、改发 `[材料体量]` synthetic 让父代理逐份派 `insight_reader`;`单份超出通读容量,已拦下`(files/limit)= 某份超 `SINGLE_DOC_LIMIT`(150KB),**分治也救不了**(子代理与父同模型同窗口),正常发送弹 toast 让用户拆分、drain 路径只打日志(无 toast 通道)。`mention file size unavailable` = `@` 引用文件的 `readFileBuffer` 失败,该份按 0 字节计入(读不到的文件本就内联不进上下文)。<br>**「传了一堆 md 第一轮就超限」先看这条**:没有 `内联预算超限` 说明判定没触发(材料总量确实在预算内 / 文件被 `isTextInlineFile` 排除了),有这条却仍超限则是父代理没照 `[材料体量]` 派子代理、自己硬读——查 `[octo:assistant]` 看它调了什么。 |
