...
| `[octo:task]` | [index.tsx](../packages/app/octoapp/pages/insight/index.tsx) · [utils/task-refresh.ts](../packages/app/octoapp/pages/insight/utils/task-refresh.ts) | 长任务卡片:切会话清状态、刷新/终止/打开产物、聚合 diff。<br>⚠️ `child-session navigation blocked` **已于 2026-08-27 删除**:子会话导航改为「渲染层不给入口」(insight 的 `DataProvider` 不传 `onNavigateToSession` / `onSessionHref` → 上游 task 卡片 `clickable()` 恒 false),没有运行时判定也就没有日志。**「task 卡片还能点进子会话」不要再找这条日志**,直接看卡片右上角有没有 ↗ 图标(有 = 那两个 prop 又被传上了)。 |
| `[octo:attach]` | [utils/build-prompt-parts.ts](../packages/app/octoapp/pages/insight/utils/build-prompt-parts.ts) 判定 · [index.tsx](../packages/app/octoapp/pages/insight/index.tsx) / [utils/queue-drain.ts](../packages/app/octoapp/pages/insight/utils/queue-drain.ts) 打点 | **SPEC-INS-032 §2.6 分治判定**(发送前确定性完成,不交给模型):`内联预算超限,转子代理分治`(count/totalBytes/budget/**docCount**/**reasons**/oversized/largeDocs/unknownCount)= 本轮命中了分治判据,**一个 text/plain FilePart 都不产**、改发 `[材料体量]` synthetic 让父代理逐份派 `insight_reader`。<br>**「这批材料为什么分治 / 为什么没分治」看 `reasons`**:`text-budget` = 文本类总字节 > 32KB;`doc-count` = office/pdf 份数 ≥ 3;`doc-size` = 单份 office 二进制 > 2MB;三者皆空 = 本轮维持内联。两套口径互不换算(文本用字节、文档用份数,理由见 spec §2.6.1),所以**一堆 docx 不会因为「体积大」而触发 text-budget**——2026-08-27 之前正是这里漏了 office,9 份 docx 一个 `[材料体量]` 都不产。<br>`单份超出单次通读量,将走切段`(files/limit)= 某份超 `SINGLE_DOC_LIMIT`(150KB),**照样派子代理**,只是子代理调 `extract_document` 时会拿到切段清单按段读(§2.4 v3;v2 那版「弹 toast 让用户拆分」已删除)。`mention file size unavailable` = `@` 引用文件的 `readFileBuffer` 失败,该份按 0 字节计入(读不到的文件本就内联不进上下文)。<br>**「传了一堆材料第一轮就超限」先看这条**:没有 `内联预算超限` 就是判定没触发(看 `reasons` 与 `docCount` 对不对得上);有这条却仍超限,则是父代理没照 `[材料体量]` 派子代理、自己硬读——查 `[octo:assistant]` 看它调了什么。 |
...
| `[octo:task-queue]` | [packages/opencode/src/agent/octo-task-serialize.ts](../packages/opencode/src/agent/octo-task-serialize.ts) | **server 端插件**:insight 子代理**串行闸**([SPEC-INS-032](specs/infra/insight-subagent-dispatch.md) §5.4)。`已有子任务在跑,本次排队` = 模型一次并发派了多个 task,已被透明串成串行(**正常现象**,不是错误);`子任务持锁超时,强制释放`(error) = 某次子任务 10 分钟没回来、锁自愈,后续放行 —— 出现它说明有子任务卡死或异常退出,值得查。非 insight 会话一条都不出现(chat / make / studio 的 task 不受管)。**「模型并发派子代理撞分钟级限流」查这条**:内网全量并发只有几十个,一个用户开 3–5 路就吃掉可观比例 |
...
| `[octo:session-workdir]` | [packages/opencode/src/agent/octo-session-workdir.ts](../packages/opencode/src/agent/octo-session-workdir.ts) | **server 端插件**：insight 会话工作目录对齐（[SPEC-INS-028](specs/infra/insight-workdir-declaration.md)）。四条消息：`声明改写`（带 `before`/`after`，`Working directory` 从选中目录改成会话产物目录——**排查产物散落先看它有没有出现**）；`落点重定向`（write/edit/read 的相对 `filePath` → 产物目录，带 `tool`/`before`/`after`）；`默认目录补齐`（bash 的 `workdir` → 产物目录，glob/grep 的 `path` → 会话根，带 `key`/`value`）；三条 error：`Working directory 锚点未命中`（上游 env 模板变更的唯一信号）、`产物路径越界,拒绝改写`、`session.get 失败,保持原值`、**`根会话解析失败,退化为按当前会话取工作区`**([SPEC-INS-032](specs/infra/insight-subagent-dispatch.md) §6:沿 parentID 上溯成环或超深,带 `depth`/`err`);另有一条 info **`根会话解析`**(`sessionID`/`rootSessionID`/`depth`,只在当前会话不是根时打——即 task 子代理那种情形)。**判据自 SPEC-INS-032 起看的是「会话树根会话」的 agent**:子代理(insight_reader / general / 第三方 skill 自带的)跑在子 session 里,产物、bash cwd、glob/grep 默认范围一律归根会话目录。**根会话非 insight** 的会话树应当**一条都不出现** |
| `[octo:extract]` | [packages/opencode/src/tool/extract_document.ts](../packages/opencode/src/tool/extract_document.ts) | **server 端工具**:文档→文本抽取(docx=mammoth / pdf=unpdf / xlsx=exceljs / pptx=jszip 直抽,[SPEC-INS-016](specs/infra/insight-extract-document.md);**SPEC-INS-021 §3 起支持 txt/md 直读**,过程条 title 中文「提取文档正文:xx」)。**可见性自 [SPEC-INS-032](specs/infra/insight-subagent-dispatch.md) §3 起由 agent 权限层声明**(defaults `extract_document: deny` + octo_insight / insight_reader 显式 allow),不再是 registry 里按 agent 名硬编码的 gate。`ok`:path/format/chars/tokenEstimate/ms/**saved**(落盘的解析件路径,空 = 没落上)/**inlined**(是否把全文一并回灌)/pages·sheets·slides;`failed`:path/reason(`not-found`·`unsupported`·`parse-error`)/format/err;**`persist-failed`**:path/dir/err(SPEC-INS-016 v2 全量落盘写盘失败,**已降级为整篇返回**,功能不中断)。**「模型说文档内容不全 / 反复重读同一份文档」先看这三条**:`inlined:false` 说明正文只在 `saved` 那个文件里(设计如此,模型应 grep/read 取用);`saved:""` + `persist-failed` 才是真出问题(退回 v1 的字节截断行为)。**「上传几个文件就上下文超限」查这条的 `format`**:出现 `format:"txt"`/`"md"` 且该文件在会话 `[附件]` 区块里 = 同一份正文进了两次上下文(FilePart 已内联 + 本工具又回灌一份),即 [SPEC-INS-021 §3.1](specs/infra/insight-toolset-convergence.md) 那条提示词软约束漏了——**漏网率就是「要不要上插件硬拦」的决策数据**;研究工具(chip)那一轮本就不该出现任何 `[octo:extract]`([SPEC-INS-017](specs/infra/insight-mcp-explicit-entry.md) §8-9 已 gate 掉),出现即 gate 没生效;**`root-session-unresolved`**:落盘目录本应归**会话树根会话**([SPEC-INS-032](specs/infra/insight-subagent-dispatch.md) §6:子代理与父会话共用一个 `extracted/`),出现这条表示上溯失败(会话读不到 / 超深 / 成环)、已退化为按当前会话落盘——现象是子代理的解析件散在子会话目录里、父代理事后 grep 不到 |
| `[octo:kb]` | [packages/opencode/src/tool/knowledge_search.ts](../packages/opencode/src/tool/knowledge_search.ts) · [pages/insight/utils/account.ts](../packages/app/octoapp/pages/insight/utils/account.ts) | **server 端工具**:内网知识库检索(新接口 `queryKnowledge`,**不传库名 = 全量库**,Q3 定案),**SPEC-INS-030 起 gate 到 octo_insight**(原 chat 的 octo_ai)。另有一条 `account missing` 出在客户端 DevTools(发送时取不到工号)。spec 见 [specs/agents/insight-knowledge-search.md](specs/agents/insight-knowledge-search.md)(§4.2 接口 / §7 Q3),旧接口细节见 [chat-knowledge-search.md](specs/agents/chat-knowledge-search.md) |
| `[octo:report]` | [packages/opencode/src/tool/insight_report_search.ts](../packages/opencode/src/tool/insight_report_search.ts) | **server 端工具**:内网**研究报告库**检索(接口 `queryReportKnowledge`,与 `knowledge_search` **同 host 不同 path**、**无库名参数**——接口自身绑定报告库),与 `[octo:kb]` 并列的另一条检索链路,同样只 gate 到 `octo_insight`、chip turn 关闭。字段与 `[octo:kb]` 三条对齐,`parsed` 多一个 `downloadUrls`。spec 见 [specs/agents/insight-report-search.md](specs/agents/insight-report-search.md) |
| `[octo:ctx]` | [packages/opencode/src/tool/get_session_identity.ts](../packages/opencode/src/tool/get_session_identity.ts) | **server 端工具**:会话身份工具 `get_session_identity`([SPEC-INS-033](specs/agents/insight-session-identity.md)),只在**缺字段**时打,只记缺了哪些字段、**不打身份值**。`identity missing`(error,带 `sessionID`/`missing`)= `ctx.extra.userId` 为空,工具已返回「未获取到当前登录身份」显式失败——userId 来自 renderer `localStorage.userInfo.userId`,经 `promptAsync.extra` 透传,同时出现客户端 `[octo:kb] account missing` = `userInfo` 整体缺失(登录态问题);只有本条 = `userInfo` 里有 account 没有 userId。`identity partial`(warn,`missing:["account"]`)= 缺 account,已省略该行、正常返回。**完全无 `[octo:ctx]` 日志不代表没调用**(身份齐全时不打日志),要确认是否调用看会话里的工具调用记录 |
...
> - `parsed`:`totalDocs`/`titles`——检索成功但答非所问时看命中文档(新接口无 `_score`、按返回序,故不再打 `topScores`)。
> - `检索失败 url=…`(error):网络层失败(连不上 / 超时 / abort),带完整 url。
>
> `[octo:report]` 五条(全部出在 server 进程,不在客户端 DevTools;SPEC-INS-034)——**与 `[octo:kb]` 逐条同构**,排查思路照搬上面那组,只记差异:
> - `account missing`(error,带 `sessionID`/`query`):与 kb 同义,**出现即表示没发 HTTP 请求**。工号来源同一条 `extra.account` 管道,故两个前缀的 account 问题**必然同时出现**——只有一边出,先怀疑是不是只调了一个工具。
> - `config`:字段与 kb 完全一致(`envBaseUrl`/`usingMockDefault`/`resolvedBase`/`url`/`account`/`query`)。**两个工具共用同一个 `OCTO_KB_BASE_URL`**,只有 `url` 末段不同(`queryReportKnowledge` vs `queryKnowledge`)——内网 404 时先拿这个 `url` 和能跑通的地址逐字比,别急着怀疑 env(env 若错,`[octo:kb]` 会一起错)。
> - `response`:同 kb。
> - `parsed`:`totalDocs`/`titles` 之外**多一个 `downloadUrls`**(与 `titles` 同序)。**这是 `downloadUrl` 唯一的可观测点**——本期它只解析存进 `metadata.sources`、UI 零改动(真实数据恒为 null,没有可验的地址,下载交互属后续项,spec §4/§10.1),所以"界面上看不出来"是**预期**,不是 bug;要确认解析对不对只能看这一行。`null` / 键缺失 / 空串一律记为 `undefined`。
> - `检索失败 url=…`(error):同 kb。
> - **完全无 `[octo:report]` 日志** = 模型没调这个工具。除了 kb 那几条常规原因(是否 octo_insight、是否 chip turn——`buildToolGate` 同样关掉了它),多一条**三入口串台**要查:问题被模型判给了 `knowledge_search`(看有没有 `[octo:kb]`)或判成"读用户材料"(看有没有 `extract_document` 调用)。边界写在工具 description 与 `octo_insight.md` 两处,改措辞时两处要一起改。
