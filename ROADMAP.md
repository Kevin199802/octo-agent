...
| 序 | 规模 | spec / 任务 | 依赖 | 状态 |
|---|---|---|---|---|
| A | `[M]` | **本地工作目录布局**（worktree 文档本地化）[SPEC-INS-014](docs/specs/infra/insight-worktree-layout.md) — `insight/sources`(拷贝落地) + `insight/outputs`(扁平)、产物落点 `.octo/downloads`→`insight/outputs` 迁移、S3 上传改 lazy(预置发送时传，插件/契约不变)。隐含"MCP 只经预置触达" | 无（建于 SPEC-INS-012 projectDir 之上）| **草案已立**，待评审 / 实现 |
| B | `[S~M]` | **`extract_document` tool** — office(docx/pdf/xlsx)→文本，顺带返回字数/token 估算（测量机制） | A | 待起草 |
