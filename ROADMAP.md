...
| B2 | `[M]` | **本地解析 v1（长上下文直喂）**[SPEC-INS-018](docs/specs/infra/insight-local-analysis-v1.md) — 前提已变（内网 GLM 5 128K）：观点解析先行,单文档直喂 + 多文档 task 子代理分治 + 超阈值走 MCP chip;输出契约以出处引用为一等公民;工具集补 read/grep/write（顺带修 Truncate 引导 Read/Grep 落空的矛盾）;护栏策略随此落地;评测内网闭环（用研同学评分表） | A、B、B1 | **草案已立** |
