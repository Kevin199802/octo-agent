...
// Patch Electron binary Info.plist so macOS menu bar shows "OctoAI" in dev mode.
...
await $`plutil -replace CFBundleDisplayName -string "OctoAI" ${plist}`.quiet()
await $`plutil -replace CFBundleName -string "OctoAI" ${plist}`.quiet()
